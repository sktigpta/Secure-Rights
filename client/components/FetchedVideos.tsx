...
const FetchedVideos = ({ onNotification }: FetchedVideosProps) => {
  ...
  const fetchStoredVideos = async () => {
    try {
      const response = await fetch("https://backend.securerights.app/api/youtube/stored-videos")
  ...
  const addNewVideos = async () => {
    setRefreshing(true)
    try {
      const response = await fetch("https://backend.securerights.app/api/youtube/get-videos", { method: "POST" })
