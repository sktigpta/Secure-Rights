...
const PermittedVideos = ({ onNotification }: PermittedVideosProps) => {
  ...
  const fetchData = async () => {
    try {
      const videosRes = await axios.get("https://backend-gamma-ivory-42.vercel.app/api/permissions/permitted-videos")
  ...
  const handleAddVideo = async () => {
    ...
    try {
      const res = await axios.post("https://backend-gamma-ivory-42.vercel.app/api/permissions/permitted-videos", {
        videoId: newVideoId,
      })
  ...
  const handleDeleteVideo = async (id, title) => {
    try {
      await axios.delete(`https://backend-gamma-ivory-42.vercel.app/api/permissions/permitted-videos/${id}`)
